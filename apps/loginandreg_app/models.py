...
class LoginManager(models.Manager):
    def basic_validator(self, postData):
        ...
        if Login.objects.filter(email = postData['email']):
    ...
    def loginValidator(self, postData):
        errors = {}
        try: 
            login = Login.objects.get(email = postData['email'])
        except:
            errors['email'] = f'No email matching {postData["email"]}.'
            return errors

        if not bcrypt.checkpw(postData["password"].encode(), login.password.encode()):
